...
per-document codemap semantic validation baselines (document digest + accepted Arcana snapshot)
...
Object state, runtime state, and internal refs are implementation-owned and should not be hand-edited while commands or watchers are active. The TOML files under `.ddocs/schemas/` and `.ddocs/document-schemas/` are the deliberate exceptions: both are editable policy inputs, although document-specific files are initially generated by explicit user decisions. Link identity, immutable-frontmatter records, shared-schema history, and codemap semantic validation baselines use `refs/ddocs/state`; suggestion decisions and applied-change history use `refs/ddocs/review`. Neither creates commits on the user's normal Git branch.

Normal state and review-reference writes do not compact the private Git object
store automatically. The daemon and CLI may access `.ddocs` from separate
processes, so pack replacement remains disabled until reads and writes share a
cross-process lock. Loose objects are retained rather than risking missing
packs, state roots, review commits, or undo snapshots.

## Rebuildability

The filesystem and authored repository remain the primary rebuild source. Much of `.ddocs/` can be reconstructed, but deleting it loses historical evidence used for move reconciliation, codemap semantic validation baselines, and daemon/runtime ownership. Losing a codemap semantic baseline does not damage authored maps; it removes the prior-snapshot comparison point until a new baseline is accepted.
