...
Machine-readable output is not implied unless a command explicitly documents such a format. The stable schema-1 `check --output-format json` contract currently covers links and documentation indexes, individually or together; its schema, codes, severity values, and compatibility policy are defined in [Machine-Readable Diagnostics](machine-readable-diagnostics.md).
