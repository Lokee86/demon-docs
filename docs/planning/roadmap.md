...
- **Complete format-surface cache identity:** frontmatter and document-format cache identities are now independent, and ordinary body edits no longer invalidate frontmatter validation. Document format still uses the raw whole-document SHA-256 for its own identity. Replace that with a schema-selection and heading-structure fingerprint, then refresh only affected subsystem records after known generated rewrites.
