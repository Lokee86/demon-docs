...
An orphan is a managed Markdown document in the configured documentation tree that has no inbound link from another managed document. Links from configured folder indexes count because indexes are valid authored or generated navigation. Orphan detection is read-only: it reports navigation gaps and makes `check` fail, but it does not guess where a document should be linked or remove the file.
...
- the document linking to itself; and
- links originating from draft/stub documents.

Configured folder-index links do count. This treats maintained navigation as real reachability while preventing unfinished drafts from masking a disconnected canonical document.
...
A clean check means every normal managed Markdown document has at least one inbound link from another managed document, including configured folder indexes but excluding draft and self-links.
...
Run `ddocs fix --links` so the current index link is recorded in the link plan, then rerun the check. If the document is still reported, inspect the index target for an unresolved or malformed path.
