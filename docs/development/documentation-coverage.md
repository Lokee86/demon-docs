...
| `internal/validationcache/` | Durable normalized-path cache records and content/policy/schema identity invalidation for clean validation | [Validation Cache](../architecture/validation-cache.md), [Managed Files and State](../reference/managed-files-and-state.md) |
...
| Codemap semantic validation baseline | [Codemap Pipeline](../architecture/codemap-pipeline.md) | Accepted Arcana snapshot, document digest, deterministic mapped-node diff, stale persistence across untouched fixes, and rebaseline after document review/edit. |
...
| Package | Responsibility | Canonical current docs |
| --- | --- | --- |
| `internal/codemap/` | Existing-section extraction, normalized entries, generic semantic target resolution, datasets, authored-section stripping, unified managed-section adoption, schema-placement seam, and syntax-preserving rendering | [Codemap Extraction and Dataset](../architecture/codemap-extraction-and-dataset.md), [Codemap Managed Execution](../architecture/codemap-managed-execution.md), [Managed Files and State](../reference/managed-files-and-state.md) |
| `internal/codemaparcana/` | Arcana/Lexicon snapshot alignment, source-freshness gating, JSONL protocol transport, exact file/symbol target resolution, bounded per-document relationship projection, and snapshot-diff semantic staleness | [Codemap Extraction and Dataset](../architecture/codemap-extraction-and-dataset.md), [Codemap Corpus and Adapters](../architecture/codemap-corpus-adapters.md), [Codemap Pipeline](../architecture/codemap-pipeline.md) |
| `internal/codemapcorpus/` | Repository files, code-intelligence and per-document relationship-provider boundaries, local fallback dependency/symbol adapters, history, and related-document facts | [Codemap Corpus and Adapters](../architecture/codemap-corpus-adapters.md), [Codemap Pipeline](../architecture/codemap-pipeline.md) |
| `internal/evidence/` | Deterministic mention, structural, dependency, semantic-relationship, history, related-document, and symbol evidence | [Codemap Missing-Link Evidence](../codemap-evidence.md), [Codemap Evidence and Ranking](../architecture/codemap-evidence-and-ranking.md) |
| `internal/codemaprecommend/` | Production admission, deterministic candidate roles, scoring, score-banded coverage selection, fan-out discount, negative-evidence filtering, output bounds, and tiers | [Codemap Missing-Link Algorithm](../codemap-suggestion-algorithm.md), [Codemap Evidence and Ranking](../architecture/codemap-evidence-and-ranking.md) |
| `internal/codemapsemantic/` | Durable per-document semantic validation baselines stored in `.ddocs` | [Codemap Pipeline](../architecture/codemap-pipeline.md), [Managed Files and State](../reference/managed-files-and-state.md) |
| `internal/codemaprun/` | Production dataset/corpus planning, semantic-baseline comparison, decline replay, optional pruning, per-document plans, and transaction publication | [Codemap Managed Execution](../architecture/codemap-managed-execution.md), [Codemap Pipeline](../architecture/codemap-pipeline.md) |
