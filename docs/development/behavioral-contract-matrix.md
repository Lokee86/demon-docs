...
| Normal constructors keep private-object compaction disabled until cross-process reader/writer locking exists; explicit compaction preserves referenced state/review/undo objects | [Private Object Repository](../architecture/private-object-repository.md) | `TestDefaultCompactionIsDisabled`, explicit compaction and review-retention tests | `go test ./internal/ddrepo ./internal/review -count=1` |
...
| Arcana target resolution trusts only Arcana/Lexicon-aligned snapshots, verifies path-qualified source content, gates global symbols on current clean prepared Git state, and degrades unavailable/stale semantics explicitly | [Codemap Extraction and Dataset](../architecture/codemap-extraction-and-dataset.md) | `semantic_target_resolution_test.go`, `internal/codemaparcana/state_test.go`, `resolver_test.go`, `client_test.go`, `internal/codemaprun/target_resolver_test.go` | `go test ./internal/codemap ./internal/codemaparcana ./internal/codemaprun ./internal/app -count=1` |
| Arcana relationship evidence is collected only from currently visible exact file/verified-symbol seeds, bounded to allowlisted one-hop current-source relationships, isolated from holdout targets, and cannot independently promote a hard link | [Codemap Corpus and Adapters](../architecture/codemap-corpus-adapters.md), [Codemap Evidence and Ranking](../architecture/codemap-evidence-and-ranking.md) | `relationship_intelligence_test.go`, `internal/codemaparcana/relationships_test.go`, `internal/codemaprun/relationship_provider_test.go`, semantic relationship evidence/ranking tests | `go test ./internal/codemapcorpus ./internal/codemaparcana ./internal/evidence ./internal/codemaprecommend ./internal/codemaprun -count=1` |
| Codemap semantic staleness compares an unchanged document only against its accepted prior Arcana baseline, reports mapped-node disappearance/move/metadata/ownership/relationship change without pruning authority, and does not advance an untouched stale baseline on `fix` | [Codemap Pipeline](../architecture/codemap-pipeline.md), [Codemap Managed Execution](../architecture/codemap-managed-execution.md) | `internal/codemaparcana/staleness_test.go`, `internal/codemapsemantic/baseline_test.go`, `internal/codemaprun/semantic_staleness_test.go`, codemap inspection output test | `go test ./internal/codemaparcana ./internal/codemapsemantic ./internal/codemaprun ./internal/app -count=1` |
| Every admitted recommendation receives one deterministic role; role never changes numeric score, normal cutoff coverage stays within score bands, and hard-link allocation is capped by role/directory | [Codemap Evidence and Ranking](../architecture/codemap-evidence-and-ranking.md), [Codemap Missing-Link Algorithm](../codemap-suggestion-algorithm.md) | `internal/codemaprecommend/roles_test.go`, `selection_test.go`, precision role-validation/breakdown tests | `go test ./internal/codemaprecommend ./internal/codemapbench ./internal/codemapprecision ./internal/app -count=1` |
| Corpus adapters emit only recognized local deterministic facts and preserve authored file/directory/pattern/symbol provenance without flattening broader targets into file expansion seeds | [Codemap Corpus and Adapters](../architecture/codemap-corpus-adapters.md) | `TestAuthoredTargetsPreserveAbstractionLevel`, `TestVisibleAuthoredTargetsPreservesNonHoldoutPatternCoverage`, corpus dependency, symbol, path, history, and related-document tests | `go test ./internal/codemapcorpus ./internal/evidence -count=1` |
| Code-intelligence providers are fact-only, receive caller cancellation, and cannot publish paths outside the current repository inventory | [Codemap Corpus and Adapters](../architecture/codemap-corpus-adapters.md) | `TestBuildContextUsesInjectedCodeIntelligenceProvider`, `TestBuildContextRejectsProviderFactsOutsideRepositoryFiles`, `TestBuildContextPropagatesProviderError`, cancellation coverage | `go test ./internal/codemapcorpus ./internal/codemaprun ./internal/app -count=1` |
...
| Recommendation role, ranking, admission, caps, fan-out discounting, negative-evidence filters, and tiers are deterministic | [Codemap Evidence and Ranking](../architecture/codemap-evidence-and-ranking.md) | `internal/codemaprecommend/roles_test.go`, `suggestions_test.go`, evidence symbol tests, benchmark compatibility tests | pinned source-report comparison plus `go test ./internal/codemaprecommend ./internal/codemapbench -count=1` |
...
| Existing links are retained by default; only non-declined `hard_link` recommendations auto-add; `context` remains non-mutating; confidence pruning occurs only under explicit independent settings | [Codemap Managed Execution](../architecture/codemap-managed-execution.md), [Configuration Reference](../reference/configuration.md) | `TestBuildKeepsContextRecommendationOutOfManagedSection`, `TestBuildAddsHardLinkRecommendation`, `TestReconcileManagedRemovesOnlySelectedEntry`, pruning cases | `codemap inspect` and dry-run against representative repository docs plus full suite |
