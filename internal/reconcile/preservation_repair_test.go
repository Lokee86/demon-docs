...
func TestStaleEntryRemovalPreservesUnmanagedContent(t *testing.T) {
	...
	if strings.Contains(text, "gone.md") || strings.Contains(text, "stale.md") || strings.Contains(text, "gone/README.md") {
...
func TestParentLinksCanBeRemovedWithoutChangingBody(t *testing.T) {
	root := filepath.Join(t.TempDir(), "docs")
	child := filepath.Join(root, "guide", "README.md")
	write(t, filepath.Join(root, "README.md"), "# Documentation")
...
func TestExistingDescriptionsAndRootDisplayTitleRemainStable(t *testing.T) {
	...
	write(t, filepath.Join(root, "alpha.md"), "Parent index: [Docs](./README.md)\n\nAlpha body")
	...
	child := plannedText(t, result, filepath.Join(root, "guide", "README.md"))
	requireContains(t, child, "Parent index: [Docs](../README.md)")
