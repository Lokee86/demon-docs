...
func demonAcquire(args []string, out, errOut io.Writer) int {
	...
	if helpRequested(args) {
		fmt.Fprintln(out, "usage: demon acquire --client NAME [PATH]\n       ddocs demon acquire --client NAME [PATH]\n\nRegister an MCP, Codex, Hermes, or other external agent feeder. The caller must refresh the returned token before feeder expiry and release it when the session ends.")
		return 0
	}
...
func demonHeartbeat(args []string, out, errOut io.Writer) int {
	...
	if helpRequested(args) {
		fmt.Fprintln(out, "usage: demon heartbeat --token TOKEN [PATH]\n       ddocs demon heartbeat --token TOKEN [PATH]\n\nRefresh an external agent feeder and recover the repository demon if its owner lease is missing or stale.")
		return 0
	}
	path, code := parseDemonPath(args, fs)
	if code != 0 || *token == "" {
...
func demonRelease(args []string, out, errOut io.Writer) int {
	...
	token := fs.String("token", "", "external feeder token")
	if helpRequested(args) {
		fmt.Fprintln(out, "usage: demon release --token TOKEN [PATH]\n       ddocs demon release --token TOKEN [PATH]\n\nRelease one external agent feeder. The demon remains active while other feeders exist and otherwise stops after its grace period.")
		return 0
	}
	path, code := parseDemonPath(args, fs)
	if code != 0 || *token == "" {
