...
func addCommon(fs *flag.FlagSet, c *commonFlags) {
	...
	fs.BoolVar(&c.all, "a", false, "reconcile all configured systems")
	fs.BoolVar(&c.all, "all", false, "reconcile all configured systems")
...
func treeUsage(command string) string {
	usage := fmt.Sprintf("usage: ddocs %s [-h] [-a] [-d] [-i] [--frontmatter] [--format] [-l] [-r] [--root PATH] [--config PATH]\n", command) +
...
func runTree(ctx context.Context, command string, args []string, out, errOut io.Writer) int {
	...
	if outputFormat != "text" && outputFormat != "json" {
		fmt.Fprintln(errOut, treeUsage(command))
		fmt.Fprintf(errOut, "ddocs %s: error: argument --output-format: invalid choice: %q (choose from text, json)\n", command, outputFormat)
		return 2
	}
	...
	features := selectedFeatures(command, flags, c)
	if outputFormat == "json" && (command != "check" || (!features.Links && !features.Indexes && !features.Frontmatter && !features.Format) || features.Reverse) {
		fmt.Fprintln(errOut, treeUsage(command))
		fmt.Fprintf(errOut, "ddocs %s: error: --output-format json currently supports check with only --links, --indexes, --frontmatter, and/or --format selected\n", command)
		return 2
	}
	...
	if outputFormat == "json" {
		...
		if err := writeDiagnosticReport(out, command, exitCode, indexResult, frontmatterPlan, formatPlan, linkPlan, orphanDocuments); err != nil {
			return fail(errOut, err)
		}
		return exitCode
	}
...
func selectedFeatures(command string, flags commonFlags, c config.Config) watch.Features {
	defaultSelection := !flags.all && !flags.docsOnly && !flags.indexesOnly && !flags.frontmatterOnly && !flags.formatOnly && !flags.linksOnly && !flags.reverseOnly
	linksSelected := defaultSelection || flags.all || flags.linksOnly
	indexesSelected := defaultSelection || flags.all || flags.docsOnly || flags.indexesOnly
	frontmatterSelected := flags.all || flags.docsOnly || flags.frontmatterOnly || (defaultSelection && command != "fix")
	formatSelected := flags.all || flags.docsOnly || flags.formatOnly || (defaultSelection && command != "fix")
	reverseConfigured := len(c.ReverseIndex.Roots) > 0 || len(flags.reverseRoots.values) > 0 || len(flags.codemapHeadings.values) > 0
	if defaultSelection || flags.all {
