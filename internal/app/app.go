...
type commonFlags struct {
	...
	docsOnly, frontmatterOnly, formatOnly              bool
...
func Run(ctx context.Context, args []string, out, errOut io.Writer) int {
	...
	default:
		fmt.Fprintln(errOut, topUsageLine)
		choices := "init, status, mv, new, format, schema, fix, check, watch, codemap, suggestions, changes, config, index, links, demon"
...
func treeUsage(command string) string {
	usage := fmt.Sprintf("usage: ddocs %s [-h] [-d] [--frontmatter] [--format] [-l] [-r] [--root PATH] [--config PATH]\n", command) +
...
func selectedFeatures(flags commonFlags, c config.Config) watch.Features {
	defaultSelection := !flags.docsOnly && !flags.frontmatterOnly && !flags.formatOnly && !flags.linksOnly && !flags.reverseOnly
...
const (
	topUsageLine    = "usage: ddocs [-h] [-v] {init,status,mv,new,format,schema,fix,check,watch,codemap,suggestions,changes,config,index,links,demon} ..."
...
func show(w io.Writer, c config.Config, path string) {
	...
	fmt.Fprintf(w, "[reverse_index]\nroots = %s\n[codemap]\nheadings = %s\n[review]\nundo_depth = %d\nundo_max_age_days = %d\n", list(c.ReverseIndex.Roots), list(c.Codemap.Headings), c.Review.UndoDepth, c.Review.UndoMaxAgeDays)
