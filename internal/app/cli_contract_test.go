...
func TestCLIFileAndRenderingOverrides(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			args := append([]string{"fix", "-i", "--root", root, "--no-local-config", "--no-global-config"}, test.args...)
...
func TestCLIParentLabelAndBooleanOverrides(t *testing.T) {
	...
	var stdout, stderr bytes.Buffer
	args := []string{"fix", "-i", "--root", root, "--no-local-config", "--no-global-config", "--parent-label", "Up", "--parent-link-indexed-files", "--no-parent-link-folder-indexes"}
	if code := Run(context.Background(), args, &stdout, &stderr); code != 0 {
		t.Fatalf("code=%d stderr=%q", code, stderr.String())
	}
