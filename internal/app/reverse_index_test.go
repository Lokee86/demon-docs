...
func TestReverseIndexCheckFixAndWatchOnce(t *testing.T) {
	...
	withWorkingDirectory(t, repositoryRoot, func(string) {
		...
		out.Reset()
		errOut.Reset()
		if code := Run(context.Background(), []string{"reverse-index", "fix"}, &out, &errOut); code != 0 || !strings.Contains(out.String(), "updated 1 file") {
			t.Fatalf("fix code=%d out=%q err=%q", code, out.String(), errOut.String())
		}
		out.Reset()
		errOut.Reset()
		if code := Run(context.Background(), []string{"reverse-index", "check"}, &out, &errOut); code != 0 || !strings.Contains(out.String(), "check passed") {
			t.Fatalf("clean check code=%d out=%q err=%q", code, out.String(), errOut.String())
		}
		out.Reset()
		errOut.Reset()
		if code := Run(context.Background(), []string{"reverse-index", "watch", "--once"}, &out, &errOut); code != 0 || !strings.Contains(out.String(), "watch updated 0 file") {
