...
doc-ledger is a repo-local documentation index maintenance tool for Space Rocks.
...
Run the tool from `tools/doc-ledger/`:
...
When no config file is supplied, defaults preserve the current Space Rocks behavior.
...
Run the doc-ledger tests from the repo-local tool directory:
...
The repo also keeps Python cache artifacts out of commits with `.gitignore`, and the hygiene test fails if `tools/doc-ledger` contains `__pycache__`, `.pyc`, or `.pyo` files.
...
The docs tree uses `docs/!README.md` as its index instead of a plain `docs/README.md`.
If you are exploring the wider project docs, start at the docs tree index and follow the owning README for the topic you need.
