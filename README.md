...
Unchanged clean frontmatter and document-format results can be reused from durable `.ddocs/` cache records. Frontmatter reuse is keyed to its raw leading block, policy, selected schema, immutable snapshot, and validation engine; ordinary body edits no longer invalidate it. Document format owns separate identity, policy, and schema fields, although its current identity remains the whole-document hash until structural heading fingerprinting is added. A standalone read-only check does not initialize `.ddocs/` merely to save cache data.
...
Cold frontmatter and document-format validation, link-inventory reads, and changed Markdown source reads and parsing now use bounded worker pools. Frontmatter cache identity is isolated from body-only edits. Document format still reparses a changed file and still uses whole-document identity until structural heading fingerprinting is added. These remaining performance boundaries are tracked in [Current Product Limitations](docs/limits/current-limitations.md) and the [Roadmap](docs/planning/roadmap.md).
