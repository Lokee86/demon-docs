...
Existing configured sections are supported, and the public codemap command uses the selected effective document schema to create a missing required section at its deterministic schema position. A document whose schema does not declare a codemap section remains unchanged.
