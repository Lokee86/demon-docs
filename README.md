...
Cold frontmatter and document-format validation, link-inventory reads, and changed Markdown source reads and parsing now use bounded worker pools. Frontmatter and format cache identities are isolated from unrelated body edits. Format cache candidates still require reading the file, parsing selection metadata, and scanning its heading structure; shared command snapshots and metadata-assisted no-read hits remain later optimizations. These remaining performance boundaries are tracked in [Current Product Limitations](docs/limits/current-limitations.md) and the [Roadmap](docs/planning/roadmap.md).
